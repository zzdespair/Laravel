                                ...
                                <td>
                                    <form class="layui-form" action="">
                                        <div class="layui-form-item status" style="margin:0;">
                                            <input type="checkbox" class="check" name="status" title="启用" lay-filter="top" @if($item->castatus == 1) checked @endif>
                                            <input type="hidden" name="id" value="{{$item->caid}}">
                                        </div>
                                    </form>
                                </td>
                                <td>{{date("Y-m-d H:i:s",$item->cacreatetime)}}</td>
                                <td>@if($item->caupdatetime){{date("Y-m-d H:i:s",$item->caupdatetime)}}@else无@endif</td>
                                <td>
                                    <a href="{{route('category.edit',$item->caid)}}">    
					                   <button class="layui-btn layui-btn-small layui-btn-normal"><i class="layui-icon">&#xe642;</i></button>
                                    </a>
				                </td>
                                <td>
				                    <a href="{{action('Admin\CategoryController@destroy',['id'=>$item->caid])}}" class="delete">
