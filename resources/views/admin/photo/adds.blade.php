...
@section("content")
    ...
    <fieldset id="dataConsole" class="layui-elem-field layui-field-title">
        <legend>照片添加(支持批量上传)</legend>
    </fieldset>
    
    <form class="layui-form" action="" id="photo" empty="multipart/form-data">
        {{ csrf_field() }}
        <div class="layui-upload" style="margin-left:7%;">
            <input class="layui-btn" type="file" id="scroll_img" multiple="multiple" name="phphoto[]">
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">标题</label>
            <div class="layui-input-block">
                <input type="text" name="phtitle" lay-verify="title" autocomplete="on" placeholder="请输入文章标题" class="layui-input" style="width:50%;">
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block" style="width:45%;">
                <select name="phalid" lay-filter="aihao">
                    @foreach($album as $item)
                        <option value="{{$item->alid}}">{{$item->alname}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="checkbox" checked="" name="phstatus" lay-skin="switch" lay-filter="switchTest" lay-text="启用|禁用">
            </div>
        </div>
        <div class="layui-form-item">
            <div class="layui-input-block">
    ...
    <script>
        $(function(){
            $("#btn").click(function(){
                ...
                $.ajax({
                    url:"{{action('Admin\PhotoController@store')}}",
                    type:'post',
                    data:formData,
                    dataType: 'json',
                    async:true,
