...
@section("style")
    <script src="{{URL::asset('admins/js/label/label.js')}}"></script>
    <script>
        // console.log(  .;$$                     
        //                 ....:;$$$$$$$   $                     
        //             ..;;;$$$            $:..                  
            ...
            <div id="articleIndexTop">
                <form class="layui-form layui-form-pane" action="">
                    <div class="layui-form-item" style="margin:0;margin-top:15px;">
                        ...
                        <div class="layui-inline">
                            <div class="layui-input-inline" style="width:auto">
                                <a href="{{action('Admin\LabelController@create')}}" id="addArticle" class="layui-btn layui-btn-normal">添加标签</a>
                            </div>
                        </div>
                    </div>
                </form>
                                ...
                                <td>
                                    <form class="layui-form" action="">
                                        <div class="layui-form-item status" style="margin:0;">
                                            <input type="checkbox" class="check" name="status" title="启用" lay-filter="top" @if($item->lastatus == 1) checked="true" @endif>
                                            <input type="hidden" name="id" value="{{$item->laid}}">
                                        </div>
                                    </form>
                                </td>
                                <td>{{date("Y-m-d H:i:s",$item->lacreatetime)}}</td>
                                <td>@if($item->laupdatetime){{date("Y-m-d H:i:s",$item->laupdatetime)}}@else无@endif</td>
                                <td>
                                    <a href="{{route('label.edit',$item->laid)}}">    
					                   <button class="layui-btn layui-btn-small layui-btn-normal"><i class="layui-icon">&#xe642;</i></button>
                                    </a>
				                </td>
                                <td>
				                    <a href="{{route('label.destroy',['id'=>$item->laid])}}" class="delete">
